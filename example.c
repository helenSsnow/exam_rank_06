...
int main(int ac, char **av) {
    ...
    // start server
    int server_sock = socket(AF_INET, SOCK_STREAM, 0); //получаем фд сокета
    ...
    if ((bind(server_sock, (const struct sockaddr *)&addr, sizeof(addr))) < 0) //устанавливаем соединение
        fatal_error();
    if (listen(server_sock, SOMAXCONN) < 0) //создаем очередь из соединений
        fatal_error();

    while (1) {
        ready_for_read = ready_for_write = active_socks;
        if (select(max_sock + 1, &ready_for_read, &ready_for_write, NULL, NULL) < 0) // ожидает изменение статуса сокетов
            continue ;

        for (int sel_sock = 0; sel_sock <= max_sock; sel_sock++) {

            if (FD_ISSET(sel_sock, &ready_for_read) && sel_sock == server_sock) { // возвращает ненулевое значение, если указанный
                                                                                //файловый дескриптор присутствует в наборе и ноль
                int client_sock = accept(server_sock, (struct sockaddr *)&addr, &addr_len); //создаёт для общения с клиентом новый сокет и возвращает его дескриптор
